...
async function addDeliverableToStep(stepNum, opId) {
    const step = MDMP_STEPS.find(s => s.num === stepNum);
    const title = `${step.name} - ${new Date().toISOString().split('T')[0]}`;

    try {
        showInfo('Creating deliverable...');

        // For now, we'll store it in memory and show a modal to create the content
        // In a real system, this would create a markdown file in the appropriate folder
...
${step.name} deliverable for this operation.
...
`;
        ...
        if (!currentMDMPProducts) currentMDMPProducts = [];
        currentMDMPProducts.push({
            ...
            filePath: `${step.folder}/${title.replace(/\s+/g, '_')}.md`,
        ...
        showError('Failed to add deliverable');
    }
}
...
async function deleteDeliverable(opId, productId, stepNum) {
    ...
    try {
        ...
        currentMDMPProducts = currentMDMPProducts.filter(p => p.id !== productId);
        ...
        showError('Failed to delete deliverable');
    }
}
...
async function exportDeliverable(opId, productId) {
    try {
        ...
        const response = await fetch(`/api/export/annex?name=${encodeURIComponent(product.title)}&operation=${opId}`);
        if (response.ok) {
        ...
        showError('Failed to export deliverable');
    }
}
